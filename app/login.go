...
var (
	...
	durationSession = 10 * 24 * time.Hour
)
...
func Callback(ctx *server.Context) error {
	...
	user, err := github.NewClient(token).User(ctx.Context())
	if err != nil {
		return New500HTTPError(err)
	}
	bytes, err := json.Marshal(user)
	if err != nil {
		return New500HTTPError(xerrors.WithWrapper(ErrCookieUser, err))
	}
	ctx.SetCookie(userCookieName, string(bytes), durationSession)
